...
export default function NavItem({ item, level, pathDirect, hideMenu }: ItemType) {
    ...
    <List component="li" disablePadding key={item?.id && item.title}>
      <Link href={item.href ?? "#"} passHref style={{ textDecoration: "none" }}>
        ...
        >
          ...
          <ListItemText>
            {hideMenu ? "" : <strong>{`${item?.title}`}</strong>}
            <br />
            {item?.subtitle ? <Typography variant="caption">{hideMenu ? "" : item?.subtitle}</Typography> : ""}
          </ListItemText>

          {!item?.chip || hideMenu ? null : (
            <Chip color={item?.chipColor} variant={item?.variant ? item?.variant : "filled"} size="small" label={item?.chip} />
          )}
        </ListItemStyled>
      </Link>
    </List>
