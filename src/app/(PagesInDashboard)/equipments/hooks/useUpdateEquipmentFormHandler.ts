...
const useUpdateEquipmentFormHandler = (defaultValues: DefaultValues<IUpdateEquipmentFormInput>) => {
  const { updateEquipment } = useEquipment();
  const { controlAndErrors, submit, errorMessage, isValid, router, reset } = useFormHandler({
    defaultValues,
    onSubmit: async (data: IUpdateEquipmentFormInput) => {
  ...
  return {
    ...
    isSuccess: updateEquipment.isSuccess,
    isValid,
    router,
    reset,
  };
...
export default useUpdateEquipmentFormHandler;
