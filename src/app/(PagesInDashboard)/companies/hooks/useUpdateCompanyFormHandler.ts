...
const useUpdateCompanyFormHandler = (defaultValues: DefaultValues<IUpdateCompanyFormInput>) => {
  const { updateCompany } = useCompany();
  const { controlAndErrors, submit, errorMessage, isValid, router, reset } = useFormHandler({
    defaultValues,
    onSubmit: async (data: IUpdateCompanyFormInput) => {
  ...
  return {
    ...
    isSuccess: updateCompany.isSuccess,
    isValid,
    router,
    reset,
  };
...
export default useUpdateCompanyFormHandler;
