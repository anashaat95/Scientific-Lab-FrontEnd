...
const useAddCompanyFormHandler = (defaultValues: DefaultValues<IAddCompanyFormInput>) => {
  const { addCompany } = useCompany();
  const { controlAndErrors, submit, errorMessage, isValid, router, reset } = useFormHandler({
    defaultValues,
    onSubmit: async (data: IAddCompanyFormInput) => {
      return await addCompany.mutateAsync(data);
    },
    validationSchema,
  });
...
export default useAddCompanyFormHandler;
