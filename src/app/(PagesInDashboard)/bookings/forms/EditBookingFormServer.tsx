...
import { eBookingStatus, IBooking, IEditBookingFormInput } from "../bookingsInterfaces";
...
const EditBookingFormServer = async ({ id }: { id: string }) => {
  ...
  return (
    <Suspense fallback={<CustomLoader page={true} />}>
      <EditBookingForm
        ...
        statuses={statuses}
        yesOrNo={yesOrNo}
        booking={ConvertBookingToEditBookingForm(booking)}
      />
    </Suspense>
  );
};

const ConvertBookingToEditBookingForm = (booking: IBooking): IEditBookingFormInput => {
  return {
    id: booking.id,
...
export default EditBookingFormServer;
