...
const useEditUserFormHandler = (defaultValues: DefaultValues<IEditUserFormInput>) => {
  const { editUser } = useUser();
  const { controlAndErrors, submit, errorMessage, isValid, router, reset } = useFormHandler({
    defaultValues,
    onSubmit: async (data: IEditUserFormInput) => {
...
export default useEditUserFormHandler;
