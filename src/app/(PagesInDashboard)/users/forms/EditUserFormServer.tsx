...
import { IEditUserFormInput, IUser } from "../usersInterfaces";
import { getUserByIdService } from "../usersServicesBackEnd";
import EditUserForm from "./EditUserForm";

const EditUserFormServer = async ({ id }: { id: string }) => {
  ...
  return (
    <Suspense fallback={<CustomLoader page={true} />}>
      <EditUserForm {...data} user={ConvertUserToEditUserForm(user)} />
    </Suspense>
  );
};

const ConvertUserToEditUserForm = (user: IUser): IEditUserFormInput => {
  return {
    id: user.id,
...
export default EditUserFormServer;
