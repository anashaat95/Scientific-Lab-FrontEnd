...
const useAddLabFormHandler = (defaultValues: DefaultValues<IAddLabFormInput>) => {
  const { addLab } = useLab();
  const { controlAndErrors, submit, errorMessage, isValid, router, reset } = useFormHandler({
    defaultValues,
    onSubmit: async (data: IAddLabFormInput) => {
      return await addLab.mutateAsync(data);
    },
    validationSchema,
  });
  ...
  return {
    ...
    submit,
    errorMessage,
    isPending: addLab.isPending,
    isSuccess: addLab.isSuccess,
    isValid,
    router,
    reset,
  };
...
export default useAddLabFormHandler;
