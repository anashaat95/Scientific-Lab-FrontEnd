...
const useEditLabFormHandler = (defaultValues: DefaultValues<IEditLabFormInput>) => {
  const { editLab } = useLab();
  const { controlAndErrors, submit, errorMessage, isValid, router, reset } = useFormHandler({
    defaultValues,
    onSubmit: async (data: IEditLabFormInput) => {
      await editLab.mutateAsync(data);
    },
    validationSchema,
  });
  return {
    ...
    submit,
    errorMessage,
    isPending: editLab.isPending,
    isSuccess: editLab.isSuccess,
    isValid,
    router,
    reset,
  };
...
export default useEditLabFormHandler;
